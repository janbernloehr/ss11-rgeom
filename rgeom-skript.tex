...
= \GL_n$.
\item \index{Pullback-Bündel}\textit{Pullback-Bündel}. Sei $f: N\to M$ eine
...
\end{lem}
\begin{proof}
%Die Projektion $\pi: P\to M$  erfüllt
...
$\Leftarrow$: Gegeben sei eine lokale Trivialisierung $\phi_U: \pi^{-1}(U)\to
...
$S^7$ parallelisierbar. Außerdem ist jede Lie-Gruppe parallelisierbar.\bsp
\end{ex}
...
%TODO: Nachtragen, Notation für HFB \pi: P\to M, P \to M. 
...
\begin{prop}
Sei $\lambda: H\to G$ ein Gruppenhomomorphismus von Lie-Gruppen, $\rho: G\to
...
\end{align*}
mit $\rho = \id$ und $\lambda = i: \O_n\opento \GL_n$.\bsp
\end{ex}
...
Sei $\gamma: I\to M$ eine Kurve in $M$. Eine Kurve $\gamma^* : I\to P$ heißt
...
\begin{prop}
Sei $\gamma: I\to M$ eine Kurve in $M$, $t_0\in I$ und $u\in P_{\gamma(t_0)}$.
...
Sei $\pi: P\to M$ ein $G$-Hauptfaserbündel mit Zusammenhang $A$ und $G$ wirke
...
Sei $I=[a,b]$ ein Intervall und $\gamma: I\to M$ eine Kurve in $M$.
...
\begin{align*}
\tau: T_{\gamma(t_0)}M \to T_{\gamma(t)}M,\qquad Z_0\mapsto Z(t),
\end{align*}
...
Translation von Vektoren gegeben.\bsp
\end{ex}
...
\begin{defn}
\index{Geodätische}
Eine \emph{Geodätische} ist eine Kurve $\gamma: I\to M$ mit $\ddot{\gamma} = 0$,
d.h.
\begin{align*}
...
Kurve $\gamma:I\to M$ die Geodätengleichung $\ddot\gamma=0$  als
\begin{align*}
...
also zwei verschiedene Geodätische eine >>kürzere<< und eine >>längere<<.\bsp
\end{exenum}
\end{ex}
...
\gamma = \sigma,
\end{align*}
und $\gamma$ selbst ist ein Großkreis.\bsp
\end{ex}
...
geschlossene Kurve  $\gamma: I\to M$ durch den Punkt $x\in M$ besitzt für jedes $p\in
